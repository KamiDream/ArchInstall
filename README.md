...
按交互提示依次完成 14 个步骤。**分区完成后全程自动执行**（格式化→挂载→安装系统无需额外确认）：

| 步骤 | 内容 | 交互 |
|------|------|------|
| 0 | 风险确认 + 依赖检查 | `[y/N]` |
| 1 | 选择目标磁盘 → 固件类型 → 引导加载程序 | 输入/选择 |
| 2 | 选择安装模式（Clean / Reinstall） | 选择 |
| 3 | 查找现有 /home 分区（仅 Reinstall 模式） | 输入 |
| 4 | 确认破坏性操作 | `[y/N]` |
| 5 | 确保 GPT 分区表 | 自动 |
| 6 | 分区（含布局预览） | 输入分区大小 |
| 7 | 格式化 + 挂载 + fstab 参考 | **自动** |
| 8 | 安装基础系统（pacstrap + 时区/locale） | **自动** |
| 9 | 配置主机名 | 输入 |
| 10 | 安装引导加载程序 | **自动** |
| 11 | 启用系统服务 + initramfs + sudo | **自动** |
| 12 | 设置 root 密码 | 输入/可跳过 |
| 13 | 创建普通用户 + 完成重启 | 输入/可跳过 |

> **步骤 6→7→8 一气呵成**：分区→格式化→挂载→安装系统，中间零确认。
