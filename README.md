...
`install.sh` 是一个交互式 Arch Linux 自动化安装脚本，用于替代 `archinstall`，提供从磁盘分区、格式化、基础系统安装到系统配置的完整安装流程。脚本采用 ↑/↓ 方向键菜单导航 + `case` 语句驱动的步骤执行，分区→格式化→挂载→安装系统全自动串联。
...
pacstrap 失败时直接返回菜单，用户可检查网络后重新选择该步骤重试。
...
通过 ↑/↓ 方向键菜单导航，依次完成 9 个步骤。**选择 "Install & Partition" 后全程自动执行**（分区→格式化→挂载→安装系统无需额外确认）：

| 步骤 | 内容 | 交互 |
|------|------|------|
| 0 | 风险确认 + 依赖检查 | `[y/N]` |
| 1 | 选择目标磁盘 → 固件类型 → 引导加载程序 | ↑/↓ 选择 |
| 2 | Install & Partition（安装模式→分区→格式化→挂载） | ↑/↓ 选择 + 输入 |
| 3 | 安装基础系统（pacstrap + 时区/locale） | **自动** |
| 4 | 配置主机名 | 输入 |
| 5 | 安装引导加载程序 | **自动** |
| 6 | 启用系统服务 + initramfs + sudo | **自动** |
| 7 | 设置 root 密码 | 单键确认/可跳过 |
| 8 | 创建普通用户 + 完成重启 | 单键确认/可跳过 |
